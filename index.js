    ...
    while (filaQ0.length > 0 || filaQ1.length > 0 || filaIO.length > 0 || filaTransicao.length > 0) {
        ...
        if (filaTransicao.length > 0) {
            for (var i = 0; i < filaTransicao.length; i++) {
                ...
                i--;
                console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " foi para fila " + processo.filaDestino);
            ...
            for (var i = 0; i < filaQ1.length; i++) {
                ...
                filaQ1[i].tempoEsperando++
                    ...
                    if (filaQ1[i].tempoEsperando >= 30) {
                        ...
                        i--;
                        console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " foi para filaTransicao depois de " + processo.tempoEsperando + " na filaQ1");
        ...
        if (filaIO.length > 0) {
            ...
            console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + filaIO[0].id + " executando em filaIO");
            ...
            if (filaIO[0].tempoExecutando >= 20) {
                ...
                console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " executou em IO por " + processo.tempoExecutando + " e irá para fila de transição faltando " + processo.nrIO + " operações de IO");
                processo.tempoExecutando = 0;
                //Definir fila destino
                processo.filaDestino = 'filaQ0';
                //Colocar o processo na fila de transição
        ...
        if (filaQ0.length > 0) {
            ...
            console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + filaQ0[0].id + " executando em filaQ0");
            //Rodar o primeiro processo por 1 ms (menor unidade de tempo)
            ...
            if (filaQ0[0].tempoExecutando >= filaQ0[0].surtoCPU) {
                ...
                if (processo.nrIO > 0) {
                    ...
                    console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " foi para fila de transição depois de Q0");
                    //Definir fila destino
                    ...
                    console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " finalizado em " + processo.tempoFinal);
                }
            } //Verifica se o processo já rodou tempo suficiente no Round-Robin
            else if (filaQ0[0].tempoRR >= 10) {
                //Retirar o processo da filaQ0
                processo = filaQ0.shift();
                console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " foi para fila de transição por preempção de tempo depois de execução total " + processo.tempoExecutando);
                //Zerar o tempoRR
                processo.tempoRR = 0;
                //Definir fila destino
            ...
            if (filaQ1.length) {
                //Informar que o processo começou a execução
                console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + filaQ1[0].id + " executando em filaQ1");
                //Rodar o primeiro processo por 1 ms (menor unidade de tempo)
                ...
                if (filaQ1[0].tempoExecutando >= filaQ1[0].surtoCPU) {
                    ...
                    if (processo.nrIO > 0) {
                        //Coloca o processo na filaIO
                        console.log("Tempo:[" + (countTempo - 1) + "-" + countTempo + "]" + " Processo " + processo.id + " foi para fila de transição depois de Q1");
                        //Definir fila destino
                        processo.filaDestino = 'filaIO';
                        //Colocar o processo na fila de transição
                        filaTransicao.push(processo);
